        ...
        <>
            <div className='bxcuntWrrapper'>
                <div className='boxcount'>
                    <h2>{formatNumber(dashData?.totalAmount) || formatNumber(0)}</h2>
                    <p>Total Amount</p>
                </div>
                <div className='boxcount'>
                    <h2>{formatNumber(dashData?.deduction) || formatNumber(0)}</h2>
                    <p>Deduction</p>
                </div>
                <div className='boxcount'>
                    <h2>{formatNumber(dashData?.earning) || formatNumber(0)}</h2>
                    <p>Earning</p>
                </div>
                <div className='boxcount'>
                    <h2>{formatNumber(dashData?.settledAmount) || formatNumber(0)}</h2>
                    <p>Settled Amount</p>
                </div>
                <div className='boxcount'>
                    <h2>{formatNumber(dashData?.unsettledAmount) || formatNumber(0)}</h2>
                                ...
                                <table className="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        {paginData?.list?.map((order, index) => (
                                            <tr key={index}>
                                                <td>
                                                    {formatNumber((Number(page) == 1 ? 0 : (Number(page) - 1) * paginData.itemsCountPerPage) + index + 1)}
                                                </td>
                                                <td><Moment format="DD-MM-yyyy">{order?.createdAt}</Moment></td>
                                                <td>Dr. {order?.doctorId?.firstName} {order?.doctorId?.lastName}</td>
                                                <td>{formatNumber(order?.totalAmount) || formatNumber(0)}</td>
                                                <td>{formatNumber((order?.totalDeductedAmount || 0) + (order?.totalAdminCommission || 0)) || formatNumber(0)}</td>
                                                <td>{formatNumber(order?.totalEarnings) || formatNumber(0)}</td>
                                                <td>{formatNumber(order?.totalSettledAmount) || formatNumber(0)}</td>
                                                <td>{formatNumber(order?.totalUnsettledAmount) || formatNumber(0)}</td>
                                                <td>
                                                    {order?.totalUnsettledAmount > 0 && (

                                                        <ReactConfirm
                                                            type="custom"
                                                            name='Sattle'
                                                            value={order?.doctorId?._id}
                                                            route={`/earnings/settle`}
                                                            action={() => {
                                                                getData();
                                                                getDashData();
                                                            }}
                                                            message={`Are you sure you want to settle amount of ${formatNumber(order?.totalUnsettledAmount)}?`}
                                                            method="POST"
                                                            payload={{ doctorId: order?.doctorId?._id, type:'referral' }}
                                                        />
                                                    )}
                                                    <button
                                                        className="mx-1 btn btn-secondary btn-sm"
                                                        onClick={() => navigate(`/transactions/view/${order?.doctorId?._id}/1`)}
                                                    >
                                                        View Transaction
                                                    </button>
                                                </td>
