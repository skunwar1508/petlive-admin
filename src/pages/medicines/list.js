...
function List() {
  ...
  const pageHasChanged = (pageNumber) => {
    if (pageNumber !== paginData.activePage) {
      ...
      navigate(`/medicines/list/${pageNumber}?searchString=${searchString || ''}`);
    }
  };
            ...
            <div className="card-header d-flex align-items-center justify-content-between py-3 w-100 bg-transparent">
              <div className="left-widget col-4">
                <SearchFormik name="getFilter" onChange={(values) => {
                  ...
                  navigate(`/medicines/list/${1}?` + queryString);
                }} />
              </div>
              <div className="right-widget ml-auto col-4">
                <div className="float-right d-flex align-items-center">
                  <Link to={`/medicines/add`} className="btn-custom btn-theme">Add</Link>
                </div>
              </div>
            </div>
                        ...
                        <tr>
                          ...
                          <td>
                            <ReactConfirm
                              ...
                              route={`/medicine/status/${data?._id}`}
                              action={() => getData()}
                          ...
                          <td className='text-right'>
                            <ReactConfirm method="POST" type='delete' route={`/medicine/delete/${data?._id}`} action={() => getData()} message='Are you sure you want to delete this category?' />
                          </td>
