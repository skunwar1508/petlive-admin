...
function List() {
  ...
  const pageHasChanged = (pageNumber) => {
    if (pageNumber !== paginData.activePage) {
      ...
      navigate(`/pettype/list/${pageNumber}?searchString=${searchString || ''}`);
    }
  };
            ...
            <div className="card-header d-flex align-items-center justify-content-between py-3 w-100 bg-transparent">
              <div className="left-widget col-4">
                <SearchFormik name="getFilter" onChange={(values) => {
                  ...
                  navigate(`/pettype/list/${1}?` + queryString);
                }} />
              </div>
              <div className="right-widget ml-auto col-4">
                <div className="float-right d-flex align-items-center">
                  <Link to={`/pettype/add`} className="btn-custom btn-theme">Add</Link>
                </div>
              </div>
            </div>
                      ...
                      <React.Fragment key={key}>
                        <tr>
                          ...
                          <ReactConfirm
                            type="switch"
                            value={data?.status}
                            route={`/category/status/${data?._id}`}
                            action={() => getData()}
                          ...
                          <td className='text-right'>
                            <Link to={`/pettype/edit/${data?._id}`} className="btn btn-sm btn-primary mr-2">Edit</Link>
