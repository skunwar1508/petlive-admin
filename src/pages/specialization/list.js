...
function List() {
  ...
  const getData = async () => {
    ...
    await authAxios({
      method: 'POST',
      url: `/admin/specialization/paginate`,
      data: postData,
    })
      .then((res) => {
        setPaginData({
  ...
  useEffect(() => {
    getData();
  }, [page, searchString, type, paginData.sort]);
  ...
  const pageHasChanged = (pageNumber) => {
    if (pageNumber !== paginData.activePage) {
      ...
      navigate(`/specialization/${type}/list/${pageNumber}?searchString=${searchString || ''}`);
    }
  };
            ...
            <div className="card-header d-flex align-items-center justify-content-between py-3 w-100 bg-transparent">
              <div className="left-widget col-4">
                <SearchFormik name="getFilter" onChange={(values) => {
                  ...
                  navigate(`/specialization/${type}/list/${1}?` + queryString);
                }} />
              </div>
              <div className="right-widget ml-auto col-4">
                <div className="float-right d-flex align-items-center">
                  <Link to={`/specialization/${type}/add`} className="btn-custom btn-theme">Add</Link>
                </div>
              </div>
            </div>
                ...
                <table className="table table-hover mb-0">
                  ...
                  <tbody>
                    {paginData?.list?.length > 0 && paginData?.list?.map((data, key) => (
                      <React.Fragment key={key}>
                        <tr>
                          ...
                          {/* {type == 'secondary' && <td>{data?.parent?.title}</td>} */}
                          <td>{data?.title}</td>
                          <td>
                          <ReactConfirm
                            type="switch"
                            value={data?.isEnabled}
                            route={`/admin/specialization/status/${data?._id}`}
                            action={() => getData()}
                          ...
                          <td className='text-right'>
                            <ReactConfirm type='delete' route={`/admin/specialization/${data?._id}`} action={() => getData()} message='Are you sure you want to delete this specialization?' />
                          </td>
                    ...
                    {paginData?.list?.length == 0 && (
                      <tr>
                        <td colSpan={5} className='text-center'>Unable to load specializations. Please try again later.</td>
                      </tr>
                      )}
                  </tbody>
                </table>
