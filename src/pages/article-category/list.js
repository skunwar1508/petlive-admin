...
function List() {
  ...
  const getData = async () => {
    ...
    await authAxios({
      method: 'POST',
      url: `/admin/article-category/paginate`,
      data: postData,
    })
      .then((res) => {
        setPaginData({
  ...
  const pageHasChanged = (pageNumber) => {
    if (pageNumber !== paginData.activePage) {
      ...
      navigate(`/article-category/list/${pageNumber}?searchString=${searchString || ''}`);
    }
  };
            ...
            <div className="card-header d-flex align-items-center justify-content-between py-3 w-100 bg-transparent">
              <div className="left-widget col-4">
                <SearchFormik name="getFilter" onChange={(values) => {
                  ...
                  navigate(`/article-category/list/${1}?` + queryString);
                }} />
              </div>
              <div className="right-widget ml-auto col-4">
                <div className="float-right d-flex align-items-center">
                  <Link to={`/article-category/add`} className="btn-custom btn-theme">Add</Link>
                </div>
              </div>
            </div>
                      ...
                      <React.Fragment key={key}>
                        <tr>
                          ...
                          <ReactConfirm
                            ...
                            route={`/admin/article-category/status/${data?._id}`}
                            action={() => getData()}
                          ...
                          <td className='text-right'>
                            <ReactConfirm type='delete' route={`/admin/article-category/delete/${data?._id}`} action={() => getData()} message='Are you sure you want to delete this category?' />
                          </td>
