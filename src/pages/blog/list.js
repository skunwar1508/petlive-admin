...
function List() {
  ...
  const pageHasChanged = (pageNumber) => {
    if (pageNumber !== paginData.activePage) {
      ...
      navigate(`/blog/list/${pageNumber}?${queryString}`);
    }
  };
            ...
            <div className="card-header d-flex align-items-center justify-content-between py-3 w-100 bg-transparent">
              <div className="left-widget col-4">
                <SearchFormik name="getFilter" onChange={(values) => {
                  ...
                  navigate(`/blog/list/${1}?` + queryString);
                }} />
              </div>
              <div className="right-widget ml-auto col-4">
                ...
                <div className="float-right d-flex align-items-center">
                  <Link to={`/blog/add`} className="btn btn-theme btn-sm">
                    Create Blog
                  </Link>
                </div>
              </div>
                      ...
                      paginData?.list?.map((data, key) => (
                        <React.Fragment key={key}>
                          <tr>
                            ...
                            <td>
                              <a href={data?.coverImage?.path} target="_blank">
                                <img style={{ width: '100px', maxHeight: '100px' }} src={data?.coverImage?.path || ''} className='img-fluid' />
                              </a>
                            </td>
                            <td>{data?.title}</td>
                            <td>
                              <ViewMore data={data?.content} maxLength={20} />
                            </td>
                            <td>
                            ...
                            <td className="text-right">
                              <Link to={`/blog/edit/${data?._id}`} className="btn btn-info">
                                Edit
                              </Link>
